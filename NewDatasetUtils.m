classdef NewDatasetUtils < handle
    methods (Static)
        ...
        function status = ConvertDcm2Nii(app)
            ...
            app.dlg.Message = msg;
            app.dlg.Value =  0 / 2;
            niiFiles = dir(fullfile(app.fp, '**\*.nii'));
        ...
        function convertFolder(inpath, outpath, dcm2nii)
            ...
            cmnd = [cmnd '-p n '];      %philips precise float
        ...
        function DWI2ADC(fn)
            ...
            bvals = sort(unique(load(fn)));
            X = [-bvals' ones(length(bvals),1)];

            nii_fn = strrep(fn, '.bval', '.nii');
            nii = load_nii(nii_fn);
            ...
            %init arrays
            out_img = nii;
            out_img.img = zeros(sx*sy*sz,1);
            input_img = reshape(nii.img,sx*sy*sz,st);
            
            %Sort according to bvals
            [~, I] = sort(mean(input_img), 'descend');
            input_img = input_img(:,I);
            
            %Calculate ADC values based on solving the diffusion equation.
            %TODO: takes a while...
            for ij=1:sx*sy*sz
               if(sum(input_img(ij,:)==0) > 0)
                   continue
               end
               S = single(input_img(ij,:)');
               p = X\log(S);
               out_img.img(ij) = max(p(1),0);
            end
            out_img.hdr.dime.bitpix = 32;
            out_img.hdr.dime.datatype = 16;
            out_img.hdr.dime.scl_slope = 1;
            out_img.hdr.dime.scl_inter = 0;
            out_img.img = reshape(out_img.img,sx,sy,sz);
            out_img.hdr.dime.dim(1) = 3;
            out_img.hdr.dime.dim(5) = 1;

            [folder, name, ext] = fileparts(nii_fn);
            save_nii(out_img, fullfile(folder, ['ADC_' name ext]))
