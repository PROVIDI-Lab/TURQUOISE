...
classdef MathUtils < handle
    ...
    methods (Static)
        ...
        function [out_vol, ratio] = ResampleVolume(in_vol,in_vs,out_vs)
            if(any(in_vs ~= out_vs))
                ratio = in_vs./out_vs;
                out_vol = zeros([round(size(in_vol(:,:,:,1)).*ratio)    ...
                                 size(in_vol,4)]);
                for idx_4d = 1:size(out_vol,4)
                    V = zeros(size(out_vol(:,:,:,1)));
                    C = in_vol(:,:,:,idx_4d);
                    
%                     Speed up the case where only z is anisotropic
                    if ratio(1) == ratio(2) == 1
                        for z=1:size(out_vol,3)
                            nz = 1+floor((z-1)/ratio(3));
                            V(:,:,z) = C(:,:,nz);
                        end
                        out_vol(:,:,:,idx_4d) = V;
                        continue
                    ...
                    %If multiple axes are anisotropic
                    for x = 1:size(out_vol,1)
                        nx = 1+floor((x-1)/ratio(1));
                        for y=1:size(out_vol,2)
                            ny = 1+floor((y-1)/ratio(2));
                            for z=1:size(out_vol,3)
                                nz = 1+floor((z-1)/ratio(3));
                                V(x,y,z) = C(nx,ny,nz);
                            end
