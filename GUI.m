...
%               y = hit.Source.Parent.Parent.CurrentPoint(2);
        ...
        function MouseHover(app, column, row, axID)
            ...
            if isempty(app.data{app.imagePerAxis(axID)}); return; end

            UOId = Objects.FindUOUnderMouse(app, column, row, axID);
            
            if app.buttonDown
                %set/reset activity timer
                Interaction.ToggleInteractionTimer(app)
                GUI.MoveCrosshair(app, column, row, axID)
        ...
        function DisplayHoverValue(app, column, row, axID)
        ...
        %other ax(i/e)s along with the position.
            ...
            if nargin == 2     %axID
                axID        = varargin{1};
                column      = app.crosshairPerAxis{axID}(1);
                row         = app.crosshairPerAxis{axID}(2);
                ...
                row         = min(row, sz(2));
                row         = max(row, 1);
                xyz         = NiftiUtils.rc2xyz(app, column, row, axID);

            elseif nargin == 3     %row, column
                column      = varargin{1};
                row         = varargin{2};
                xyz         = NiftiUtils.rc2xyz(app, column, row);
            elseif nargin == 4
                column      = varargin{1};
                row         = varargin{2};
                axID        = varargin{3};
