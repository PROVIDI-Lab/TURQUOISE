    ...
    methods (Static)        
        ...
        function LoadUserObjects(app, idx)
            ...
            for file = segFiles'
                ...
                profile = IOUtils.GetProfileFromFn( ...
                    fullfile(file.folder, file.name));
                if ~strcmp(profile, app.user_profile)
                    continue
                end
        ...
        %Loads a segmentation from a .nii file and creates a userObject.
            ...
            %find profile, if any
            profile = IOUtils.GetProfileFromFn(fn);
...
%                 close(h)
        ...
        % TODO: add header checking!!
            ...
            for file_id=1:length(files)
                ...
                %Removed because load_untouch_header takes a long time for
                %some reason. Could add it back if needed. Might need to
                %add other checks as well.
        ...
        function profile = GetProfileFromFn(fn)
            %Gets the profile from a segmentation filename 
            %assumes that all segmentations are stored as:
            % **/rmsstudio/[image]/[profile]/file
        ...
        function profile = GetProfileFromFolder(fp)
            %Gets the profile from the path to a folder with segmentations 
            %assumes that all segmentations are stored as:
            % **/rmsstudio/[image]/[profile]/file
            % 'fp' should be: **/rmsstudio/[image]/[profile]
            
            [base, folder, ~] = fileparts(fp);
            [~, superfolder, ~] = fileparts(base);
            if strcmp(superfolder, 'rmsstudio') || strcmp(folder, 'rmsstudio')
                profile = ''; %no profile
            else
