classdef PrePrcReslice < handle
    methods (Static)
        ...
        function Start(app)
            %Very ugly way to gain acces to nifti functions
            addpath(...
                fullfile(...
                fileparts(fileparts(mfilename('fullpath'))),'nifti'))
            ...
            if app.MultipleimagesCheckBox.Value
                ...
                for i = 1:length(folders)
                    ...
                    PrePrcReslice.ResliceFolder(app, folder.name, d, message)
                end
            else
                %single folder
                message = ['Folder 1 out of 1'];
                PrePrcReslice.ResliceFolder(app, '', d, message)
            end

            close(d);
        end

        function ResliceFolder(app, folder, d, msg)
            files = dir(fullfile(app.filepath, folder,'*.nii'));
